package com.abaya.picacho.biz.resource.service.impl;
...
import com.abaya.picacho.biz.resource.repository.ResourceRepository;
...
public class ResourceServiceImpl implements ResourceService {
    ...
    @Override
    public boolean resourceIsValid(Long resourceId) throws ServiceException {
        if (resourceId == null) throw new ServiceException("传入的资源编号为空！");

        Resource resource = repository.findById(resourceId).orElse(new Resource());
        if (resource.getId() == null) return false;

        return resource.getState() == CommonState.valid;
    }

    @Override
    public List<Resource> queryAllValidResource() {
        return this.queryAll().stream().filter(resource -> resource.getState() == CommonState.valid).collect(Collectors.toList());
    }
}
